...
AssociationTableObject::AssociationTableObject(Memory& memory)
  ...
  Property* properties[] = {
      new DataProperty(PID_OBJECT_TYPE, false, PDT_UNSIGNED_INT, 1,
                       ReadLv3 | WriteLv0, (uint16_t)OT_ASSOC_TABLE),
      new DataProperty(PID_TABLE, false, PDT_GENERIC_04, 512,
                       ReadLv3 | WriteLv0)  // FIXME: implement correctly
  };
...
uint16_t AssociationTableObject::getTSAP(uint16_t idx) {
  if (idx >= entryCount()) return 0;

  return sys_be16_to_cpu(_tableData[2 * idx + 1]);
}

uint16_t AssociationTableObject::getASAP(uint16_t idx) {
  if (idx >= entryCount()) return 0;

  return sys_be16_to_cpu(_tableData[2 * idx + 2]);
}
...
void AssociationTableObject::prepareBinarySearch() {
  ...
  // the first n ASAP are sorted (strictly increasing number), these are
  // assigning sending TSAP the remaining ASAP have to be all repetitions,
  // otherwise we set sortedEntryCount to 0, which forces linear search
    ...
    for (uint16_t idx = 0; idx < entryCount(); idx++) {
      ...
      if (sortedEntryCount) {
        ...
        if (currentASAP > lastASAP)
          ...
          sortedEntryCount =
              idx;  // last found index indicates end of sorted list
...
int32_t AssociationTableObject::translateAsap(uint16_t asap) {
  ...
  if (sortedEntryCount) {
    ...
    while (low <= high) {
      ...
      if (asap_i == asap) return getTSAP(i);
    ...
    for (uint16_t i = 0; i < entryCount(); i++)
      if (getASAP(i) == asap) return getTSAP(i);
  }
