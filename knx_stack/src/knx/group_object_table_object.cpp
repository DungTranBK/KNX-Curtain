...
GroupObjectTableObject::GroupObjectTableObject(Memory& memory)
  ...
  Property* properties[]{
      new DataProperty(PID_OBJECT_TYPE, false, PDT_UNSIGNED_INT, 1,
                       ReadLv3 | WriteLv0, (uint16_t)OT_GRP_OBJ_TABLE)};
  TableObject::initializeProperties(sizeof(properties), properties);
}

GroupObjectTableObject::~GroupObjectTableObject() { freeGroupObjects(); }

uint16_t GroupObjectTableObject::entryCount() {
  if (loadState() != LS_LOADED) return 0;
...
bool GroupObjectTableObject::initGroupObjects() {
  ...
  for (uint16_t asap = 1; asap <= goCount; asap++) {
    ...
    if (go.valueReadOnInit()) go.requestObjectRead();
  }
...
void GroupObjectTableObject::freeGroupObjects() {
  if (_groupObjects) delete[] _groupObjects;
