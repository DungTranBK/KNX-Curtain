...
BauSystemBDevice::BauSystemBDevice(Platform &platform)
    : BauSystemB(platform), _addrTable(_memory), _assocTable(_memory),
      _groupObjTable(_memory),
  ...
  _memory.addSaveRestore(
      &_groupObjTable); // changed order for better memory management
...
ApplicationLayer &BauSystemBDevice::applicationLayer() { return _appLayer; }
...
void BauSystemBDevice::sendNextGroupTelegram() {
  ...
  for (uint16_t asap = startIdx; asap <= objCount; asap++) {
    ...
    goSecurity.toolAccess =
        false; // Secured group communication never uses the toolkey. ETS knows
               // all keys, also the group keys.
    ...
    if (flag == WriteRequest && go.transmitEnable()) {
      ...
      _appLayer.groupValueWriteRequest(AckRequested, asap, go.priority(),
                                       NetworkLayerParameter, goSecurity, data,
                                       go.sizeInTelegram());
    } else if (flag == ReadRequest) {
      _appLayer.groupValueReadRequest(AckRequested, asap, go.priority(),
                                      NetworkLayerParameter, goSecurity);
    }
...
void BauSystemBDevice::updateGroupObject(GroupObject &go, uint8_t *data,
                                         uint8_t length) {
...
bool BauSystemBDevice::configured() {
  // _configured is set to true initially, if the device was configured with ETS
  // it will be set to true after restart
...
void BauSystemBDevice::groupValueWriteLocalConfirm(
...
void BauSystemBDevice::groupValueReadLocalConfirm(
    AckType ack, uint16_t asap, Priority priority, HopCountType hopType,
    const SecurityControl &secCtrl, bool status) {
  GroupObject &go = _groupObjTable.get(asap);
...
void BauSystemBDevice::groupValueReadIndication(
  ...
  uint8_t *data = go.valueRef();
  _appLayer.groupValueReadResponse(AckRequested, asap, priority, hopType,
                                   secCtrl, data, go.sizeInTelegram());
}

void BauSystemBDevice::groupValueReadAppLayerConfirm(
    uint16_t asap, Priority priority, HopCountType hopType,
    const SecurityControl &secCtrl, uint8_t *data, uint8_t dataLength) {
  GroupObject &go = _groupObjTable.get(asap);
