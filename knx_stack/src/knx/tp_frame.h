...
#include <stdint.h>
// #include <string>  // Commented: Zephyr không có C++ string, dùng C string thay thế
...
class TpFrame
{
    ...
    public:
        ...
        inline const char* humanSource()
        {
            static char buffer[10];  // Static để return pointer
            uint16_t value = source();
            sprintf(buffer, "%02i.%02i.%03i", (value >> 12 & 0b1111), (value >> 8 & 0b1111), (value & 0b11111111));
            return buffer;
        }

        inline const char* humanDestination()
        {
            static char buffer[10];  // Static để return pointer
