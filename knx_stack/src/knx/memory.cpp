...
LOG_MODULE_REGISTER(knx_memory, LOG_LEVEL_WRN);
...
void Memory::readMemory() {
  ...
  LOG_INF(">>> readMemory: _metadataSize=%d, flashSize=%u", _metadataSize,
          flashSize);
  printHex("RESTORED ", flashStart, _metadataSize);

  uint16_t metadataBlockSize = alignToPageSize(_metadataSize);

  _freeList = new MemoryBlock(flashStart + metadataBlockSize,
                              flashSize - metadataBlockSize);
  ...
  if (_deviceObject.apiVersion == apiVersion) {
    if (_versionCheckCallback != 0) {
      versionCheck =
          _versionCheckCallback(manufacturerId, hardwareType, version);
      // callback should provide infomation about version check failure reasons
               ...
               memcmp(_deviceObject.hardwareType(), hardwareType,
                      LEN_HARDWARE_TYPE) == 0) {
      ...
      // DEBUG: Print loaded header values
      print(">>> [DEBUG] readMemory Header: API=");
  ...
  if (versionCheck == FlashTablesInvalid) {
    println(
        "TableObjects are referring to an older firmware version and are "
        "not loaded");
    return;
  }
  ...
  for (int i = 0; i < _tableObjCount; i++) {
    ...
    // this works because TableObject saves a relative addr and restores it
    // itself
...
void Memory::writeMemory() {
  LOG_WRN(">>> MEM_TRACE: Memory::writeMemory() ENTERED");
  ...
  for (int i = 0; i < _tableObjCount; i++)
    writeBufferSize = MAX(writeBufferSize,
                          _tableObjects[i]->saveSize() + 2 /*for memory pos*/);
  ...
  bufferPos =
      pushByteArray(_deviceObject.hardwareType(), LEN_HARDWARE_TYPE, bufferPos);
  bufferPos = pushWord(_deviceObject.version(), bufferPos);

  flashPos =
      _platform.writeNonVolatileMemory(flashPos, buffer, bufferPos - buffer);
  ...
  for (int i = 0; i < _tableObjCount; i++) {
    ...
    // save to size of the memoryblock for tableobject too, so that we can
    // rebuild the usedList and freeList
...
void Memory::saveMemory() {
  LOG_WRN(">>> MEM_TRACE: Memory::saveMemory() called");
  writeMemory();
}
...
void Memory::addSaveRestore(SaveRestore* obj) {
  ...
  _saveCount += 1;
  uint16_t size = obj->saveSize();
  _metadataSize += size;
  LOG_INF(">>> addSaveRestore (Obj): added %d bytes, total _metadataSize=%d",
          size, _metadataSize);
}
...
void Memory::addSaveRestore(TableObject* obj) {
  ...
  LOG_INF(
      ">>> addSaveRestore (Table): added %d+2 bytes, total _metadataSize=%d",
      size, _metadataSize);
}
  ...
  while (freeBlock) {
    if (freeBlock->size >= size) {
      if (blockToUse != nullptr &&
          (blockToUse->size - size) > (freeBlock->size - size))
...
void Memory::freeMemory(uint8_t* ptr) {
  ...
  if (_saveTimeout == 0)
    _saveTimeout =
        1;  // prevent 0=disabled; no impact by minimal increased timeout
}
...
void Memory::writeMemory(uint32_t relativeAddress, size_t size, uint8_t* data) {
  ...
  LOG_WRN("!!! Memory::writeMemory: relativeAddress=0x%08X, size=%u",
          relativeAddress, size);
  if (size > 0 && size <= 64) {
    LOG_HEXDUMP_WRN(data, size, "MEM WRITE DATA:");
  }
...
void Memory::readMemory(uint32_t relativeAddress, size_t size, uint8_t* data) {
  ...
  LOG_WRN("!!! Memory::readMemory: relativeAddress=0x%08X, size=%u",
          relativeAddress, size);
  if (size > 0 && size <= 64) {
    LOG_HEXDUMP_WRN(data, size, "MEM READ DATA:");
  }
}
...
void Memory::addToFreeList(MemoryBlock* block) {
  ...
  while (current) {
    if (current->address <= block->address &&
        (current->next == nullptr || block->address < current->next->address)) {
      // add after current
      ...
      if (current == _freeList) _freeList = block;
...
uint16_t Memory::alignToPageSize(size_t size) {
  size_t pageSize =
      4;  //_platform.flashPageSize(); // align to 32bit for now, as aligning to
          // flash-page-size causes side effects in programming
...
void Memory::addNewUsedBlock(uint8_t* address, size_t size) {
  ...
  while (smallerFreeBlock) {
    if (smallerFreeBlock->next == nullptr ||
        (smallerFreeBlock->next != nullptr &&
         smallerFreeBlock->next->address > address))
      break;
  ...
  if (smallerFreeBlock->address == address) {
    ...
    if (address + size < oldEndAddr) {
      // we take the middle part of the block, so we need a new free block for
      // the end part
...
void Memory::loop() {
  if (_saveTimeout != 0 && millis() - _saveTimeout > 30000) {
    LOG_WRN(">>> MEM_TRACE: Memory::loop() TRIGGERED (timeout=%lu, now=%lu)",
            _saveTimeout, millis());
    _saveTimeout = 0;
    writeMemory();
  }
