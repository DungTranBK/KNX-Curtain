...
#ifdef USE_USB
      ,
      _usbTunnelInterface(*this, _bau.deviceObject().maskVersion(),
                          _bau.deviceObject().manufacturerId())
#endif
{
  // The cEMI server will hand out the device address + 1 to the cEMI client
  // (e.g. ETS), so that the device and the cEMI client/server
  // connection(tunnel) can operate simultaneously.
...
void CemiServer::clientAddress(uint16_t value) { _clientAddress = value; }
...
void CemiServer::dataIndicationToTunnel(CemiFrame& frame) {
  ...
  if (isRf) {
    ...
    pushByteArray(frame.rfSerialOrDoA(), 6,
                  &data[5]);   // RF add. info:Serial or Domain Address
...
void CemiServer::handleLData(CemiFrame& frame) {
  ...
  if (_dataLinkLayer->mediumType() == DptMedium::KNX_RF) {
    ...
    if (((frame.data())[1] == 0x0A) &&  // Additional info total length: we only
                                        // handle one additional info of type RF
        ((frame.data())[2] == 0x02) &&  // Additional info type: RF
        ((frame.data())[3] ==
         0x08))  // Additional info length of type RF: 8 bytes (fixed)
    {
      frame.rfInfo((frame.data())[4]);

      // Use the values provided in the RF additonal info
      if (((frame.data())[5] != 0x00) || ((frame.data())[6] != 0x00) ||
          ((frame.data())[7] != 0x00) || ((frame.data())[8] != 0x00) ||
          ((frame.data())[9] != 0x00) || ((frame.data())[10] != 0x00)) {
    ...
    // Note: There is another link layer frame number counter inside the RF data
    // link layer class!
...
void CemiServer::handleMPropRead(CemiFrame& frame) {
  ...
  // propertyValueRead() allocates memory for the data! Needs to be deleted
  // again!
  _bau.propertyValueRead((ObjectType)objectType, objectInstance, propertyId,
                         numberOfElements, startIndex, &data, dataSize);

  // Patch result for device address in device object
  // The cEMI server will hand out the device address + 1 to the cEMI client
  // (e.g. ETS), so that the device and the cEMI client/server
  // connection(tunnel) can operate simultaneously. KNX IP Interfaces which
  // offer multiple simultaneous tunnel connections seem to operate the same
  // way. Each tunnel has its own cEMI client address which is based on the main
  // device address.
  if (((ObjectType)objectType == OT_DEVICE) &&
      (propertyId == PID_DEVICE_ADDR) && (numberOfElements == 1)) {
    data[0] = (uint8_t)(_clientAddress & 0xFF);
  } else if (((ObjectType)objectType == OT_DEVICE) &&
             (propertyId == PID_SUBNET_ADDR) && (numberOfElements == 1)) {
...
void CemiServer::handleMPropWrite(CemiFrame& frame) {
  ...
  if (((ObjectType)objectType == OT_DEVICE) &&
    ...
    _bau.propertyValueWrite((ObjectType)objectType, objectInstance, propertyId,
                            numberOfElements, startIndex, requestData,
                            requestDataSize);
  }
...
void CemiServer::handleMReset(CemiFrame& frame) {
  ...
  LOG_WRN(">>> MEM_TRACE: handleMReset calling _bau.writeMemory()");
  _bau.writeMemory();
  // Prepare response
