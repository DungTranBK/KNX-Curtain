...
#warning "KNX_FLASH_SIZE not defined, using 1024"
#endif
#endif
...
typedef uint32_t (*FlashCallbackWrite)(uint32_t relativeAddress,
...
class Platform {
 public:
  ...
  // unicast socket
  virtual bool sendBytesUniCast(uint32_t addr, uint16_t port, uint8_t* buffer,
                                uint16_t len);
  ...
  // --- Overwrite these methods in the device-plattform to use the EEPROM
  // Emulation API for UserMemory ----
  //
  // --- changes to the UserMemory are written directly into the address space
  // starting at getEepromBuffer
  ...
  virtual uint32_t writeNonVolatileMemory(uint32_t relativeAddress,
                                          uint8_t* buffer, size_t size);
  virtual uint32_t readNonVolatileMemory(uint32_t relativeAddress,
                                         uint8_t* buffer, size_t size);
  virtual uint32_t writeNonVolatileMemory(uint32_t relativeAddress,
                                          uint8_t value, size_t repeat);
  virtual void saveSequenceNumber(bool toolAccess, uint64_t seqNum) {}
  virtual uint64_t loadSequenceNumber(bool toolAccess) { return 0; }

  NvMemoryType NonVolatileMemoryType();
  void NonVolatileMemoryType(NvMemoryType type);

  // --- Overwrite these methods in the device-plattform to use flash memory
  // handling by the knx stack ---
  // --- also set _memoryType = Flash in the device-plattform's contructor
  // --- optional: overwrite writeBufferedEraseBlock() in the device-plattform
  // to reduce overhead when flashing multiple pages

  // size of one flash page in bytes
  virtual size_t flashPageSize();

#ifdef KNX_FLASH_CALLBACK
  void registerFlashCallbacks(FlashCallbackSize callbackFlashSize,
