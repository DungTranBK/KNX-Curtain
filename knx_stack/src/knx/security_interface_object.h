...
class SecurityInterfaceObject : public InterfaceObject {
 public:
  ...
  /**
   * @brief Set FDSK from external provisioning data.
   * Must be called BEFORE readMemory() to take effect on first boot.
  ...
  const uint8_t* groupKey(
      uint16_t addressIndex);  // returns group key for group address index

  uint16_t indAddressIndex(uint16_t indAddr);  // returns 1-based index of
                                               // address in security IA table

  void setSequenceNumber(bool toolAccess, uint64_t seqNum);
  uint64_t getSequenceNumber(bool toolAccess);
 ...
 private:
  ...
  uint8_t getFromFailureLogByIndex(uint8_t index, uint8_t* data,
                                   uint8_t maxDataLen);

  void errorCode(ErrorCode errorCode);

  void clearSecurityTables();
  ...
  // Our FDSK (mutable: can be set from provisioning)
