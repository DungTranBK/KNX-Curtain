...
#include <math.h>    // Thay <cmath> cho Zephyr compatibility
#include <stdlib.h>  // Thay <cstdlib> cho Zephyr compatibility
#include <string.h>  // Thay <cstring> cho Zephyr compatibility

#include "bits.h"

#define ASSERT_PAYLOAD(x) \
  if (payload_length != (x)) return false
#define ENSURE_PAYLOAD(x)

bool KNX_Decode_Value(uint8_t* payload, size_t payload_length,
                      const Dpt& datatype, KNXValue& value) {
  if (payload_length > 0) {
    // DPT 1.* - Binary
    if (datatype.mainGroup == 1 && datatype.subGroup >= 1 &&
        datatype.subGroup <= 23 && datatype.subGroup != 20 && !datatype.index)
      return busValueToBinary(payload, payload_length, datatype, value);

    // DPT 2.* - Binary Control
    if (datatype.mainGroup == 2 && datatype.subGroup >= 1 &&
        datatype.subGroup <= 12 && datatype.index <= 1)
      return busValueToBinaryControl(payload, payload_length, datatype, value);

    // DPT 3.* - Step Control
    if (datatype.mainGroup == 3 && datatype.subGroup >= 7 &&
        datatype.subGroup <= 8 && datatype.index <= 1)
      return busValueToStepControl(payload, payload_length, datatype, value);

    // DPT 4.* - Character
    if (datatype.mainGroup == 4 && datatype.subGroup >= 1 &&
        datatype.subGroup <= 2 && !datatype.index)
      return busValueToCharacter(payload, payload_length, datatype, value);

    // DPT 5.* - Unsigned 8 Bit Integer
    if (datatype.mainGroup == 5 &&
        ((datatype.subGroup >= 1 && datatype.subGroup <= 6 &&
          datatype.subGroup != 2) ||
         datatype.subGroup == 10) &&
        !datatype.index)
      return busValueToUnsigned8(payload, payload_length, datatype, value);

    // DPT 6.001/6.010 - Signed 8 Bit Integer
    if (datatype.mainGroup == 6 &&
        (datatype.subGroup == 1 || datatype.subGroup == 10) && !datatype.index)
      return busValueToSigned8(payload, payload_length, datatype, value);

    // DPT 6.020 - Status with Mode
    if (datatype.mainGroup == 6 && datatype.subGroup == 20 &&
        datatype.index <= 5)
    ...
    if (datatype.mainGroup == 13 &&
        (datatype.subGroup == 1 || datatype.subGroup == 2 ||
         (datatype.subGroup >= 10 && datatype.subGroup <= 15)) &&
        !datatype.index)
    ...
    if (datatype.mainGroup == 18 && datatype.subGroup == 1 &&
        datatype.index <= 1)
      return busValueToSceneControl(payload, payload_length, datatype, value);

    // DPT 19.* - Date and Time
    if (datatype.mainGroup == 19 && datatype.subGroup == 1 &&
        (datatype.index <= 3 || datatype.index == 9 || datatype.index == 10))
    ...
    // DPT 26.* - Scene Info
    if (datatype.mainGroup == 26 && datatype.subGroup == 1 &&
        datatype.index <= 1)
...
bool KNX_Encode_Value(const KNXValue& value, uint8_t* payload,
                      size_t payload_length, const Dpt& datatype) {
  if (datatype.mainGroup == 1 && datatype.subGroup >= 1 &&
      datatype.subGroup <= 23 && datatype.subGroup != 20 && !datatype.index)
  ...
  if (datatype.mainGroup == 2 && datatype.subGroup >= 1 &&
      datatype.subGroup <= 12 && datatype.index <= 1)
    return valueToBusValueBinaryControl(value, payload, payload_length,
                                        datatype);
  ...
  if (datatype.mainGroup == 6 && datatype.subGroup == 20 && datatype.index <= 5)
    return valueToBusValueStatusAndMode(value, payload, payload_length,
                                        datatype);
  ...
  // DPT 13.100 - Long Time Period
  if (datatype.mainGroup == 13 && datatype.subGroup == 100 && !datatype.index)
    return valueToBusValueLongTimePeriod(value, payload, payload_length,
                                         datatype);
  ...
  if (datatype.mainGroup == 18 && datatype.subGroup == 1 && datatype.index <= 1)
    return valueToBusValueSceneControl(value, payload, payload_length,
                                       datatype);
  ...
  if (datatype.mainGroup == 221 && datatype.subGroup == 1 &&
      datatype.index <= 1)
    return valueToBusValueSerialNumber(value, payload, payload_length,
                                       datatype);
  ...
  if (datatype.mainGroup == 235 && datatype.subGroup == 1 &&
      datatype.index <= 3)
    return valueToBusValueActiveEnergy(value, payload, payload_length,
                                       datatype);
  ...
  if (datatype.mainGroup == 239 && datatype.subGroup == 1 &&
      datatype.index <= 1)
    return valueToBusValueFlaggedScaling(value, payload, payload_length,
                                         datatype);
...
bool busValueToBinary(const uint8_t* payload, size_t payload_length,
                      const Dpt& datatype, KNXValue& value) {
  ASSERT_PAYLOAD(1);
...
bool busValueToBinaryControl(const uint8_t* payload, size_t payload_length,
                             const Dpt& datatype, KNXValue& value) {
  ASSERT_PAYLOAD(1);
...
bool busValueToStepControl(const uint8_t* payload, size_t payload_length,
                           const Dpt& datatype, KNXValue& value) {
  ASSERT_PAYLOAD(1);
  value = (uint8_t)(unsigned8FromPayload(payload, 0) & 0x0F);
  return true;
}
bool busValueToCharacter(const uint8_t* payload, size_t payload_length,
                         const Dpt& datatype, KNXValue& value) {
  ASSERT_PAYLOAD(1);
  int8_t charValue = signed8FromPayload(payload, 0);

  if (datatype.subGroup == 1 && (charValue & 0x80)) return false;
...
bool busValueToUnsigned8(const uint8_t* payload, size_t payload_length,
                         const Dpt& datatype, KNXValue& value) {
  ASSERT_PAYLOAD(1);
...
bool busValueToSigned8(const uint8_t* payload, size_t payload_length,
                       const Dpt& datatype, KNXValue& value) {
  ASSERT_PAYLOAD(1);
...
bool busValueToStatusAndMode(const uint8_t* payload, size_t payload_length,
                             const Dpt& datatype, KNXValue& value) {
  ASSERT_PAYLOAD(1);
...
bool busValueToUnsigned16(const uint8_t* payload, size_t payload_length,
                          const Dpt& datatype, KNXValue& value) {
  ASSERT_PAYLOAD(2);
...
bool busValueToTimePeriod(const uint8_t* payload, size_t payload_length,
                          const Dpt& datatype, KNXValue& value) {
  ASSERT_PAYLOAD(2);
...
bool busValueToSigned16(const uint8_t* payload, size_t payload_length,
                        const Dpt& datatype, KNXValue& value) {
  ASSERT_PAYLOAD(2);
...
bool busValueToTimeDelta(const uint8_t* payload, size_t payload_length,
                         const Dpt& datatype, KNXValue& value) {
  ASSERT_PAYLOAD(2);
...
bool busValueToFloat16(const uint8_t* payload, size_t payload_length,
                       const Dpt& datatype, KNXValue& value) {
  ASSERT_PAYLOAD(2);

  if (unsigned16FromPayload(payload, 0) == 0x7FFF) return false;

  value = float16FromPayload(payload, 0);
  return true;
}

bool busValueToTime(const uint8_t* payload, size_t payload_length,
                    const Dpt& datatype, KNXValue& value) {
  ASSERT_PAYLOAD(3);
...
bool busValueToDate(const uint8_t* payload, size_t payload_length,
                    const Dpt& datatype, KNXValue& value) {
  ASSERT_PAYLOAD(3);
  ...
  if (year > 99 || month < 1 || month > 12 || day < 1) return false;
...
bool busValueToUnsigned32(const uint8_t* payload, size_t payload_length,
                          const Dpt& datatype, KNXValue& value) {
  ASSERT_PAYLOAD(4);
...
bool busValueToSigned32(const uint8_t* payload, size_t payload_length,
                        const Dpt& datatype, KNXValue& value) {
  ASSERT_PAYLOAD(4);
...
bool busValueToLongTimePeriod(const uint8_t* payload, size_t payload_length,
                              const Dpt& datatype, KNXValue& value) {
  ASSERT_PAYLOAD(4);
  value = signed32FromPayload(payload, 0);
  return true;
...
bool busValueToFloat32(const uint8_t* payload, size_t payload_length,
                       const Dpt& datatype, KNXValue& value) {
  ASSERT_PAYLOAD(4);
...
bool busValueToAccess(const uint8_t* payload, size_t payload_length,
                      const Dpt& datatype, KNXValue& value) {
  ASSERT_PAYLOAD(4);
...
bool busValueToString(const uint8_t* payload, size_t payload_length,
                      const Dpt& datatype, KNXValue& value) {
  ASSERT_PAYLOAD(14);
  ...
  for (int n = 0; n < 14; ++n) {
    ...
    if (!datatype.subGroup && (value & 0x80)) return false;
  }
...
bool busValueToScene(const uint8_t* payload, size_t payload_length,
                     const Dpt& datatype, KNXValue& value) {
  ASSERT_PAYLOAD(1);
...
bool busValueToSceneControl(const uint8_t* payload, size_t payload_length,
                            const Dpt& datatype, KNXValue& value) {
  ASSERT_PAYLOAD(1);
...
bool busValueToSceneInfo(const uint8_t* payload, size_t payload_length,
                         const Dpt& datatype, KNXValue& value) {
  ASSERT_PAYLOAD(1);
...
bool busValueToSceneConfig(const uint8_t* payload, size_t payload_length,
                           const Dpt& datatype, KNXValue& value) {
  ASSERT_PAYLOAD(1);
...
bool busValueToDateTime(const uint8_t* payload, size_t payload_length,
                        const Dpt& datatype, KNXValue& value) {
  ASSERT_PAYLOAD(8);
  ...
  if (!bitFromPayload(payload, 48)) {
    switch (datatype.index) {
      case 0: {
        ...
        if ((hours > 24 || minutes > 59 || seconds > 59)) return false;
      ...
      case 1: {
        if (bitFromPayload(payload, 53)) return false;
      ...
      case 2: {
        if (bitFromPayload(payload, 50)) return false;
...
bool busValueToUnicode(const uint8_t* payload, size_t payload_length,
                       const Dpt& datatype, KNXValue& value) {
  // TODO
  return false;
}

bool busValueToSigned64(const uint8_t* payload, size_t payload_length,
                        const Dpt& datatype, KNXValue& value) {
  ASSERT_PAYLOAD(8);
...
bool busValueToAlarmInfo(const uint8_t* payload, size_t payload_length,
                         const Dpt& datatype, KNXValue& value) {
  ASSERT_PAYLOAD(6);
...
bool busValueToSerialNumber(const uint8_t* payload, size_t payload_length,
                            const Dpt& datatype, KNXValue& value) {
  ASSERT_PAYLOAD(6);
...
bool busValueToVersion(const uint8_t* payload, size_t payload_length,
                       const Dpt& datatype, KNXValue& value) {
  ASSERT_PAYLOAD(2);
...
bool busValueToScaling(const uint8_t* payload, size_t payload_length,
                       const Dpt& datatype, KNXValue& value) {
  ASSERT_PAYLOAD(3);
...
bool busValueToTariff(const uint8_t* payload, size_t payload_length,
                      const Dpt& datatype, KNXValue& value) {
  ASSERT_PAYLOAD(3);
...
bool busValueToLocale(const uint8_t* payload, size_t payload_length,
                      const Dpt& datatype, KNXValue& value) {
...
bool busValueToRGB(const uint8_t* payload, size_t payload_length,
                   const Dpt& datatype, KNXValue& value) {
  ASSERT_PAYLOAD(3);
  uint32_t rgb = (unsigned16FromPayload(payload, 0) << 8) |
                 unsigned8FromPayload(payload, 2);
  value = rgb;
  return true;
}

bool busValueToRGBW(const uint8_t* payload, size_t payload_length,
                    const Dpt& datatype, KNXValue& value) {
  ASSERT_PAYLOAD(6);
...
bool busValueToFlaggedScaling(const uint8_t* payload, size_t payload_length,
                              const Dpt& datatype, KNXValue& value) {
  ASSERT_PAYLOAD(2);
...
bool busValueToActiveEnergy(const uint8_t* payload, size_t payload_length,
                            const Dpt& datatype, KNXValue& value) {
  ASSERT_PAYLOAD(6);
...
bool valueToBusValueBinary(const KNXValue& value, uint8_t* payload,
                           size_t payload_length, const Dpt& datatype) {
  bitToPayload(payload, payload_length, 7, value);
  return true;
}

bool valueToBusValueBinaryControl(const KNXValue& value, uint8_t* payload,
                                  size_t payload_length, const Dpt& datatype) {
  unsigned8ToPayload(payload, payload_length, 0, (uint64_t)value, 0x03);
  return true;
}

bool valueToBusValueStepControl(const KNXValue& value, uint8_t* payload,
                                size_t payload_length, const Dpt& datatype) {
  unsigned8ToPayload(payload, payload_length, 0, (uint64_t)value, 0x0F);
  return true;
}

bool valueToBusValueCharacter(const KNXValue& value, uint8_t* payload,
                              size_t payload_length, const Dpt& datatype) {
  if ((uint64_t)value > INT64_C(255) ||
      (datatype.subGroup == 1 && (uint64_t)value > INT64_C(127)))
    return false;
...
bool valueToBusValueUnsigned8(const KNXValue& value, uint8_t* payload,
                              size_t payload_length, const Dpt& datatype) {
  if ((int64_t)value < INT64_C(0)) return false;

  switch (datatype.subGroup) {
    case 1: {
      if ((double)value > 100.0) return false;

      unsigned8ToPayload(payload, payload_length, 0,
                         round((double)value * 255.0 / 100.0), 0xFF);
      break;
    }

    case 3: {
      if ((double)value > 360.0) return false;

      unsigned8ToPayload(payload, payload_length, 0,
                         round((double)value * 255.0 / 360.0), 0xFF);
      break;
    }

    case 6: {
      if ((int64_t)value > INT64_C(254)) return false;
    ...
    default: {
      if ((int64_t)value > INT64_C(255)) return false;
...
bool valueToBusValueSigned8(const KNXValue& value, uint8_t* payload,
                            size_t payload_length, const Dpt& datatype) {
...
bool valueToBusValueStatusAndMode(const KNXValue& value, uint8_t* payload,
                                  size_t payload_length, const Dpt& datatype) {
  if (datatype.index < 5)
...
bool valueToBusValueUnsigned16(const KNXValue& value, uint8_t* payload,
                               size_t payload_length, const Dpt& datatype) {
...
bool valueToBusValueTimePeriod(const KNXValue& value, uint8_t* payload,
                               size_t payload_length, const Dpt& datatype) {
  struct tm tmp = value;
...
bool valueToBusValueSigned16(const KNXValue& value, uint8_t* payload,
                             size_t payload_length, const Dpt& datatype) {
  if ((int64_t)value < INT64_C(-32768) || (int64_t)value > INT64_C(32767))
    return false;

  if (datatype.subGroup == 10) {
    if ((double)value < -327.68 || (double)value > 327.67) return false;

    signed16ToPayload(payload, payload_length, 0,
                      (int16_t)((double)value * 100.0), 0xFFFF);
  } else
...
bool valueToBusValueTimeDelta(const KNXValue& value, uint8_t* payload,
                              size_t payload_length, const Dpt& datatype) {
  struct tm tmp = value;
  time_t timeSinceEpoch = mktime(&tmp);
...
bool valueToBusValueFloat16(const KNXValue& value, uint8_t* payload,
                            size_t payload_length, const Dpt& datatype) {
  ...
  if (numValue > 670433.28) return false;

  if (numValue < -671088.64) return false;

  switch (datatype.subGroup) {
    case 1:
      if (numValue < -273.0) return false;
    ...
    case 24:
    case 25:
      if (numValue < -670760.0) return false;
    ...
    case 27:
      if (numValue < -459.6) return false;
...
bool valueToBusValueTime(const KNXValue& value, uint8_t* payload,
                         size_t payload_length, const Dpt& datatype) {
  switch (datatype.index) {
    case 0: {
      if ((int64_t)value < INT64_C(0) || (int64_t)value > INT64_C(7))
      ...
      ENSURE_PAYLOAD(3);
      unsigned8ToPayload(payload, payload_length, 0, (uint64_t)value << 5,
                         0xE0);
      break;
    }
...
bool valueToBusValueDate(const KNXValue& value, uint8_t* payload,
                         size_t payload_length, const Dpt& datatype) {
  struct tm tmp = value;

  if (tmp.tm_year < 1990 || tmp.tm_year > 2089) return false;
...
bool valueToBusValueUnsigned32(const KNXValue& value, uint8_t* payload,
                               size_t payload_length, const Dpt& datatype) {
...
bool valueToBusValueSigned32(const KNXValue& value, uint8_t* payload,
                             size_t payload_length, const Dpt& datatype) {
  if ((int64_t)value < INT64_C(-2147483648) ||
      (int64_t)value > INT64_C(2147483647))
    return false;
...
bool valueToBusValueLongTimePeriod(const KNXValue& value, uint8_t* payload,
                                   size_t payload_length, const Dpt& datatype) {
  if ((int64_t)value < INT64_C(-2147483648) ||
      (int64_t)value > INT64_C(2147483647))
    return false;
...
bool valueToBusValueFloat32(const KNXValue& value, uint8_t* payload,
                            size_t payload_length, const Dpt& datatype) {
  double numValue = value;

  if (numValue < (-8388608.0 * pow(2, 255)) ||
      numValue > (8388607.0 * pow(2, 255)))
    return false;
...
bool valueToBusValueAccess(const KNXValue& value, uint8_t* payload,
                           size_t payload_length, const Dpt& datatype) {
  switch (datatype.index) {
    case 0: {
      ...
      for (int n = 0, factor = 100000; n < 6; ++n, factor /= 10)
        bcdToPayload(payload, payload_length, n,
                     ((uint64_t)value / factor) % 10);
    ...
    case 5: {
      if ((uint64_t)value > INT64_C(15)) return false;
...
bool valueToBusValueString(const KNXValue& value, uint8_t* payload,
                           size_t payload_length, const Dpt& datatype) {
  ...
  for (int n = 0; n < 14; n++) {
    if (val)
      val = strValue[n];  // string terminator 0x00 will stop further
                          // assignments and init the remainig payload with zero
...
bool valueToBusValueScene(const KNXValue& value, uint8_t* payload,
                          size_t payload_length, const Dpt& datatype) {
  if ((int64_t)value < INT64_C(0) || (int64_t)value > INT64_C(63)) return false;

  unsigned8ToPayload(payload, payload_length, 0, (uint64_t)value, 0xFF);
  return true;
}

bool valueToBusValueSceneControl(const KNXValue& value, uint8_t* payload,
                                 size_t payload_length, const Dpt& datatype) {
  switch (datatype.index) {
    case 0:
...
bool valueToBusValueSceneInfo(const KNXValue& value, uint8_t* payload,
                              size_t payload_length, const Dpt& datatype) {
  switch (datatype.index) {
    case 0:
...
bool valueToBusValueSceneConfig(const KNXValue& value, uint8_t* payload,
                                size_t payload_length, const Dpt& datatype) {
  switch (datatype.index) {
    case 0: {
      if ((int64_t)value < INT64_C(0) || (int64_t)value > INT64_C(63))
...
bool valueToBusValueDateTime(const KNXValue& value, uint8_t* payload,
                             size_t payload_length, const Dpt& datatype) {
  switch (datatype.index) {
    case 0: {
    ...
    case 1: {
      ...
      if ((int64_t)value < INT64_C(0) || (int64_t)value > INT64_C(7))
        ...
        unsigned8ToPayload(payload, payload_length, 3, (int64_t)value << 5,
                           0xE0);
      }
...
bool valueToBusValueUnicode(const KNXValue& value, uint8_t* payload,
                            size_t payload_length, const Dpt& datatype) {
  // TODO
  return false;
}

bool valueToBusValueSigned64(const KNXValue& value, uint8_t* payload,
                             size_t payload_length, const Dpt& datatype) {
  signed64ToPayload(payload, payload_length, 0, (int64_t)value,
                    UINT64_C(0xFFFFFFFFFFFFFFFF));
  return true;
}

bool valueToBusValueAlarmInfo(const KNXValue& value, uint8_t* payload,
                              size_t payload_length, const Dpt& datatype) {
  switch (datatype.index) {
    case 1: {
    ...
    case 3: {
      ...
      ENSURE_PAYLOAD(6);
      unsigned8ToPayload(payload, payload_length, datatype.index,
                         (int64_t)value, 0xFF);
      break;
    }
...
bool valueToBusValueSerialNumber(const KNXValue& value, uint8_t* payload,
                                 size_t payload_length, const Dpt& datatype) {
  switch (datatype.index) {
    case 0: {
      if ((int64_t)value < INT64_C(0) || (int64_t)value > INT64_C(65535))
...
bool valueToBusValueVersion(const KNXValue& value, uint8_t* payload,
                            size_t payload_length, const Dpt& datatype) {
  switch (datatype.index) {
    case 0: {
    ...
    case 1: {
      ...
      unsigned16ToPayload(payload, payload_length, 0, (int64_t)value << 6,
                          0x07C0);
      break;
    }
...
bool valueToBusValueScaling(const KNXValue& value, uint8_t* payload,
                            size_t payload_length, const Dpt& datatype) {
  switch (datatype.index) {
    case 0: {
      uint32_t duration = value;

      if (duration > INT64_C(65535)) return false;
    ...
    case 1: {
      if ((double)value < 0.0 || (double)value > 100.0) return false;

      unsigned8ToPayload(payload, payload_length, 2,
                         round((double)value * 255.0 / 100.0), 0xff);
      break;
    }
  }
...
bool valueToBusValueTariff(const KNXValue& value, uint8_t* payload,
                           size_t payload_length, const Dpt& datatype) {
  switch (datatype.index) {
    case 0: {
      uint32_t duration = value;
...
bool valueToBusValueLocale(const KNXValue& value, uint8_t* payload,
                           size_t payload_length, const Dpt& datatype) {
  int strl = strlen(value);

  if (strl != 2) return false;

  if (!datatype.index || (datatype.mainGroup == 231 && datatype.index == 1)) {
    ENSURE_PAYLOAD(datatype.mainGroup == 231 ? 4 : 2);
    unsigned8ToPayload(payload, payload_length, datatype.index * 2,
                       ((const char*)value)[0], 0xff);
    unsigned8ToPayload(payload, payload_length, datatype.index * 2 + 1,
                       ((const char*)value)[1], 0xff);
    return true;
  }
...
bool valueToBusValueRGB(const KNXValue& value, uint8_t* payload,
                        size_t payload_length, const Dpt& datatype) {
...
bool valueToBusValueRGBW(const KNXValue& value, uint8_t* payload,
                         size_t payload_length, const Dpt& datatype) {
  switch (datatype.index) {
    case 0:  // RGBW
    {
      uint32_t rgbw = (uint32_t)value;
      unsigned32ToPayload(payload, payload_length, 0, rgbw,
                          0xffffffff);  // RGBW
      break;
    }
    case 1:  // Mask bits
...
bool valueToBusValueFlaggedScaling(const KNXValue& value, uint8_t* payload,
                                   size_t payload_length, const Dpt& datatype) {
  switch (datatype.index) {
    case 0: {
      if ((double)value < 0.0 || (double)value > 100.0) return false;
...
bool valueToBusValueActiveEnergy(const KNXValue& value, uint8_t* payload,
                                 size_t payload_length, const Dpt& datatype) {
  switch (datatype.index) {
    case 0: {
      if ((int64_t)value < INT64_C(-2147483648) ||
...
double float16FromPayload(const uint8_t* payload, int index) {
  ...
  if (mantissa & 0x8000)
    return ((~mantissa & 0x07FF) + 1.0) * -0.01 *
           (1 << ((payload[index] >> 3) & 0x0F));
...
int64_t signed64FromPayload(const uint8_t* payload, int index) {
  return ((((uint64_t)payload[index]) << 56) & UINT64_C(0xFF00000000000000)) |
         ((((uint64_t)payload[index + 1]) << 48) &
          UINT64_C(0x00FF000000000000)) |
         ((((uint64_t)payload[index + 2]) << 40) &
          UINT64_C(0x0000FF0000000000)) |
         ((((uint64_t)payload[index + 3]) << 32) &
          UINT64_C(0x000000FF00000000)) |
         ((((uint64_t)payload[index + 4]) << 24) &
          UINT64_C(0x00000000FF000000)) |
         ((((uint64_t)payload[index + 5]) << 16) &
          UINT64_C(0x0000000000FF0000)) |
         ((((uint64_t)payload[index + 6]) << 8) &
...
uint8_t bcdFromPayload(const uint8_t* payload, int index) {
  if (index % 2) return (uint8_t)(payload[index / 2] & 0x0F);

  return (uint8_t)((payload[index / 2] >> 4) & 0x0F);
}

void bitToPayload(uint8_t* payload, size_t payload_length, int index,
                  bool value) {
  ENSURE_PAYLOAD(index / 8 + 1);
  payload[index / 8] = (payload[index / 8] & ~(1 << (7 - (index % 8)))) |
                       (value ? (1 << (7 - (index % 8))) : 0);
}
void unsigned8ToPayload(uint8_t* payload, size_t payload_length, int index,
                        uint8_t value, uint8_t mask) {
  ENSURE_PAYLOAD(index + 1);
  payload[index] = (payload[index] & ~mask) | (value & mask);
}
void signed8ToPayload(uint8_t* payload, size_t payload_length, int index,
                      int8_t value, uint8_t mask) {
  ENSURE_PAYLOAD(index + 1);
  payload[index] = (payload[index] & ~mask) | (value & mask);
}
void unsigned16ToPayload(uint8_t* payload, size_t payload_length, int index,
                         uint16_t value, uint16_t mask) {
  ENSURE_PAYLOAD(index + 2);
  payload[index] =
      (payload[index] & (~mask >> 8)) | ((value >> 8) & (mask >> 8));
  payload[index + 1] = (payload[index + 1] & ~mask) | (value & mask);
}
void signed16ToPayload(uint8_t* payload, size_t payload_length, int index,
                       int16_t value, uint16_t mask) {
  ENSURE_PAYLOAD(index + 2);
  payload[index] =
      (payload[index] & (~mask >> 8)) | ((value >> 8) & (mask >> 8));
  payload[index + 1] = (payload[index + 1] & ~mask) | (value & mask);
}
void unsigned32ToPayload(uint8_t* payload, size_t payload_length, int index,
                         uint32_t value, uint32_t mask) {
  ENSURE_PAYLOAD(index + 4);
  payload[index] =
      (payload[index] & (~mask >> 24)) | ((value >> 24) & (mask >> 24));
  payload[index + 1] =
      (payload[index + 1] & (~mask >> 16)) | ((value >> 16) & (mask >> 16));
  payload[index + 2] =
      (payload[index + 2] & (~mask >> 8)) | ((value >> 8) & (mask >> 8));
  payload[index + 3] = (payload[index + 3] & ~mask) | (value & mask);
}
void signed32ToPayload(uint8_t* payload, size_t payload_length, int index,
                       int32_t value, uint32_t mask) {
  ENSURE_PAYLOAD(index + 4);
  payload[index] =
      (payload[index] & (~mask >> 24)) | ((value >> 24) & (mask >> 24));
  payload[index + 1] =
...
void float16ToPayload(uint8_t* payload, size_t payload_length, int index,
  ...
  // above calculation causes mantissa overflow for values of the form 2^n,
  // where n>11
  if (mantissa >= 0x800) {
    exponent++;
  ...
  if (wasNegative) mantissa *= -1;

  // println(mantissa);

  signed16ToPayload(payload, payload_length, index, mantissa, mask);
  unsigned8ToPayload(payload, payload_length, index, exponent << 3,
                     0x78 & (mask >> 8));
}
void float32ToPayload(uint8_t* payload, size_t payload_length, int index,
...
void signed64ToPayload(uint8_t* payload, size_t payload_length, int index,
                       int64_t value, uint64_t mask) {
  ENSURE_PAYLOAD(index + 8);
  payload[index] =
      (payload[index] & (~mask >> 56)) | ((value >> 56) & (mask >> 56));
  payload[index + 1] =
      (payload[index + 1] & (~mask >> 48)) | ((value >> 48) & (mask >> 48));
  payload[index + 2] =
      (payload[index + 2] & (~mask >> 40)) | ((value >> 40) & (mask >> 40));
  payload[index + 3] =
      (payload[index + 3] & (~mask >> 32)) | ((value >> 32) & (mask >> 32));
  payload[index + 4] =
      (payload[index + 4] & (~mask >> 24)) | ((value >> 24) & (mask >> 24));
  payload[index + 5] =
      (payload[index + 5] & (~mask >> 16)) | ((value >> 16) & (mask >> 16));
  payload[index + 6] =
      (payload[index + 6] & (~mask >> 8)) | ((value >> 8) & (mask >> 8));
  payload[index + 7] = (payload[index + 7] & ~mask) | (value & mask);
}
void bcdToPayload(uint8_t* payload, size_t payload_length, int index,
                  uint8_t value) {
