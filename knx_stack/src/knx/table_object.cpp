...
#include "memory.h"
LOG_MODULE_REGISTER(table_obj, LOG_LEVEL_WRN);
...
TableObject::TableObject(Memory& memory, uint32_t staticTableAdr,
                         uint32_t staticTableSize)
    : _memory(memory) {
...
void TableObject::beforeStateChange(LoadState& newState) {
  LOG_WRN(
      "!!! TableObject::beforeStateChange: Current=%d, New=%d, UnloadCount=%d",
      _state, newState, _tableUnloadCount);

  if (newState == LS_LOADED && _tableUnloadCount > 0) _tableUnloadCount--;

  if (_tableUnloadCount > 0) return;

  if (newState == LS_UNLOADED) {
    _tableUnloadCount++;

    if (_beforeTablesUnload != 0) _beforeTablesUnload();
  }
}

LoadState TableObject::loadState() { return _state; }

void TableObject::loadState(LoadState newState) {
  if (newState == _state) return;

  LOG_WRN("!!! TableObject::loadState CHANGED: %d -> %d", _state, newState);
...
uint8_t* TableObject::save(uint8_t* buffer) {
  ...
  uint32_t relAddress = _data ? _memory.toRelative(_data) : 0;
  buffer = pushInt(relAddress, buffer);

  buffer = InterfaceObject::save(buffer);

  LOG_WRN("!!! TableObject::save (obj %p): Saved %d bytes. _state=%d, _size=%u, rel_addr=0x%08X",
      this, buffer - originalBuffer, _state, _size, relAddress);
  LOG_HEXDUMP_WRN(originalBuffer, buffer - originalBuffer, "SAVED TABLE_OBJ DATA:");
  return buffer;
}
...
const uint8_t* TableObject::restore(const uint8_t* buffer) {
  ...
  LOG_WRN("!!! TableObject::restore (obj %p): Restored %d bytes. _state=%d, _size=%u, rel_addr=0x%08X, _data=%p",
      this, buffer - originalBuffer, _state, _size, relativeAddress, _data);
  LOG_HEXDUMP_WRN(originalBuffer, buffer - originalBuffer, "RESTORED TABLE_OBJ DATA:");
  return buffer;
}
...
bool TableObject::allocTable(uint32_t size, bool doFill, uint8_t fillByte) {
  LOG_WRN(">>> MEM_TRACE: TableObject::allocTable(size=%u) called", size);
  LOG_WRN("!!! TableObject::allocTable: Requested Size=%u bytes", size);

  if (_staticTableAdr) {
    LOG_WRN("!!! allocTable: Failed, using static table addr.");
    return false;
  }
  ...
  if (!_data) {
    LOG_ERR("!!! allocTable FATAL: Could not allocate %u bytes (OOM)", size);
    return false;
  }
  ...
  _size = size;
  LOG_WRN("!!! TableObject::allocTable: SUCCESS, %u bytes allocated", size);
...
void TableObject::loadEventLoading(const uint8_t* data) {
  uint8_t event = data[0];
  LOG_WRN("!!! TableObject::loadEventLoading: RECV EVENT 0x%02X", event);
  ...
  switch (event) {
    ...
    case LE_LOAD_COMPLETED:
      LOG_WRN("!!! TableObject transition to LS_LOADED");
      _memory.saveMemory();
      loadState(LS_LOADED);
      break;

    case LE_UNLOAD:
      LOG_WRN("!!! TableObject transition to LS_UNLOADED");
      loadState(LS_UNLOADED);
      break;

    case LE_ADDITIONAL_LOAD_CONTROLS:
      LOG_WRN(
          "!!! TableObject (obj %p) RECV LE_ADDITIONAL_LOAD_CONTROLS (0x03)",
          this);
      additionalLoadControls(data);
      break;

    default:
      LOG_ERR("!!! TableObject ERROR: Undef load cmd 0x%02X", event);
      loadState(LS_ERROR);
      errorCode(E_GOT_UNDEF_LOAD_CMD);
  }
}
...
uint8_t* TableObject::data() { return _data; }
...
void TableObject::initializeProperties(size_t propertiesSize,
                                       Property** properties) {
  Property* ownProperties[] = {new CallbackProperty<TableObject>(
      ...
      [](TableObject* obj, uint16_t start, uint8_t count,
        ...
        LOG_WRN("!!! PID_LOAD_STATE_CONTROL READ: state=%d", obj->_state);
        data[0] = obj->_state;
        return 1;
      },
      [](TableObject* obj, uint16_t start, uint8_t count,
         const uint8_t* data) -> uint8_t {
        LOG_WRN("!!! PID_LOAD_STATE_CONTROL WRITE: event(state)=%d", data[0]);
        obj->loadEvent(data);
        return 1;
      })};
...
void TableObject::initializeDynTableProperties(size_t propertiesSize,
                                               Property** properties) {
  Property* ownProperties[] = {
      new CallbackProperty<TableObject>(
          this, PID_TABLE_REFERENCE, false, PDT_UNSIGNED_LONG, 1,
          ReadLv3 | WriteLv0,
          [](TableObject* obj, uint16_t start, uint8_t count,
             uint8_t* data) -> uint8_t {
            if (start == 0) {
      ...
      new CallbackProperty<TableObject>(
          this, PID_MCB_TABLE, false, PDT_GENERIC_08, 1, ReadLv3 | WriteLv0,
          [](TableObject* obj, uint16_t start, uint8_t count,
             uint8_t* data) -> uint8_t {
            ...
            pushByte(0xFF,
                     data + 5);  // Read access 4 bits + Write access 4 bits
