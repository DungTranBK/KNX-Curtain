...
void printFrame(TpFrame *tpframe) {
  ...
  print((tpframe->flags() & TP_FRAME_FLAG_ADDRESSED) ? 'D'
                                                     : '_');      // Recv for me
...
void __isr __time_critical_func(TpUartDataLinkLayer::processRx)(bool isr) {
  ...
  /*
   * Some platforms support the detection of whether the hardware buffer has
   * overflowed. Theoretically, you could now discard the buffer, but then a
   * valid frame may be lost. Therefore, only one piece of information is output
   * later in the loop and byte processing "tries" to respond to it.
   */
...
void TpUartDataLinkLayer::processRxByte() {
   ...
   */
  // Log byte in hex format
  if (_rxState == RX_INVALID && (millis() - _rxLastTime) > 2 &&
      !_platform.uartAvailable()) {
  ...
  if (_rxState == RX_INVALID) {
    /*
     * As soon as a frame has been processed invalidly or an unknown command
     * arrives, the status changes to RX_INVALID. From now on I must assume that
     * there has been a transmission error and the current bytes are invalid.
     * The same applies if a HW overflow is detected.
     *
     * The time of the last frame is 3ms past and there is no more data in the
     * buffer. (Is checked by me)
     * - If the marker mode is active and a U_FRAME_END_IND has been detected
     * correctly. (Checked here)
     *
  ...
  } else if ((byte & L_DATA_MASK) == L_DATA_STANDARD_IND ||
             (byte & L_DATA_MASK) == L_DATA_EXTENDED_IND) {
    /*
     * Process a previous frame if still available. This should normally only
     * occur in the bus monitor because an ACK is also being waited for here
     */
    processRxFrameComplete();
    ...
    /*
     * Here an ack is set inital without Addressed. This is used if an Ack is
     * still set from the previous frame, is set back. This happens if
     * processing is delayed too much (e.g. because no DMA/IRQ is used). The ACK
     * can be sent as often as required because it is only stored in the BCU and
     * is only used / sent when required.
     *
     * Of course, you can only do this if you are not sending yourself, as you
     * do not ACK your own frames. The BCU may ignore this, but I wanted to be
...
#ifndef NCN5120
      /*
       * Filter "Protocol errors" because this is set on other BCUs such as the
       * Siements when the timing is not correct. Unfortunately, perfect timing
       * is not possible, so this error must be ignored. Also has no known
       * effects.
       */
      _tpState &= 0b11101000;
#endif
...
void TpUartDataLinkLayer::processRxFrameByte(uint8_t byte) {
  /*
   * If the maker is active, the first U_FRAME_END_IND must be ignored and a
   * subsequent byte must be waited for. The subsequent byte is therefore
   * decisive for how this byte is to be evaluated.
   */
  ...
  /*
   * If the previous byte was a U_FRAME_END_IND and the new byte is a
   * U_FRAME_STATE_IND, then the reception is cleanly completed and the frame
   * can be processed.
   */
     ...
     * that the frame has been processed successfully. Subsequent bytes are
     * therefore clean again Control commands, even if the frame was discarded
     * due to an invalid checksum (which would mean RX_INVAID)
     */
    _rxState = RX_IDLE;
  }

  /*
   * This is a hypothetical case in which the frames are sent without markers
   * even though marker mode is active. Here the current frame is processed and
   * RX_INVALID is set, as the current byte is not processed. This case can
   * occur if the marker mode is not supported by the TPUart (NCN51xx feature)
   * but has been activated.
   */
  else if (markerMode() && _rxFrame->isFull()) {
    processRxFrameComplete();
    /*
     * RX_INVALID because theoretically the frame could have been processed as
     * valid. However, since the current byte has already been "started" to be
     * processed, it is missing in the processing chain and therefore the
     * subsequent bytes cannot be used.
     */
    _rxState = RX_INVALID;
  }
   ...
   * If marker mode is active, a U_FRAME_END_IND byte may only be processed if
   * the previous byte was also a U_FRAME_END_IND.
   */
  else if (!markerMode() || byte != U_FRAME_END_IND ||
    ...
    // If the bus monitor has been started, no processing takes place - i.e. no
    // ACKing
    if (!_monitoring) {
      // If more than 7 bytes are available, you can check whether the frame is
      // intended for "me".
      if (_rxFrame->size() == 7) {
        // Check whether I am responsible for the frame
        TPAckType ack = _cb.isAckRequired(_rxFrame->destination(),
                                          _rxFrame->isGroupAddress());

        if (_forceAck || ack) {
          /*
           * Save the responsibility that this frame is to be processed further.
           * Since there is no extra function apart from the isAckRequired, this
           * is initially treated the same. A later differentiation (possibly
           * for router mode) must then be looked at.
           */

          _rxFrame->addFlags(TP_FRAME_FLAG_ADDRESSED);

          // Of course, this is only allowed if I am not sending myself, as you
          // cannot ACK your own frames
          if (_txState == TX_IDLE) {
      ...
      // Now check whether the RxQueue still has space for Frame + Size (2) +
      // Flags(1)
      if (_rxFrame->size() == 8 &&
  ...
  /*
   * If no marker mode is active, the frame must be checked to see if it is
   * complete. isFull checks here whether the maxSize or the length
   * specification of the frame has been exceeded! In both cases, the frame must
   * be processed.
   */
...
/*
 * Processes the current frame and checks whether it is complete and valid
 * (checksum). If a frame is complete and valid, it is placed in the queue if it
 * is intended for "me" and the mode is RX_IDLE again. Otherwise the frame is
 * discarded as invalid and the status is RX_INVALID, as it is not guaranteed
 * that subsequent bytes are control codes again. Exception in marker mode, here
 * the status RX_INVALID is changed directly back to RX_IDLE at another point
 * because it is then ensured that the frame has been broken at TP level.
 */
void TpUartDataLinkLayer::processRxFrameComplete() {
  ...
  if (_rxFrame->isValid()) {
    ...
    if (_txState == TX_FRAME) {
      // check whether the receive corresponds to this: comparison of the source
      // address and destination address and start byte without taking the retry
      // bit into account
      if (!((_rxFrame->data(0) ^ _txFrame->data(0)) & ~0x20) &&
          _rxFrame->destination() == _txFrame->destination() &&
          _rxFrame->source() == _txFrame->source()) {
    ...
    // if the frame is for me or i am in busmonitor mode then i want to process
    // it further
    if (_rxFrame->flags() & TP_FRAME_FLAG_ADDRESSED || _monitoring) {
      /*
       * In bus monitor mode, you still have to wait for an Ack.
       * Therefore, the status is changed here and jumps back before the real
       * completion. As soon as another call is made (regardless of whether or
       * not the frame has been acked), the frame is closed.
       */
      ...
      // Otherwise, discard the package and release the memory -> as it is not
      // packed into the queue
    ...
    /*
     * If the frame is incomplete or invalid then switch to RX_INVALID mode as I
     * cannot distinguish, whether it is a TPBus error or a UART error or a
     * Timming error.
     */
...
/*
 * Puts the frame to be sent into a queue, as the TpUart may not yet be ready to
 * send.
 */
...
/*
 * The status should be queried regularly to detect a disconnect of the TPUart
 * and its status. In addition, the current config or mode should be transmitted
 * regularly so that after a disconnect, the TPUart is in the correct state.
 */
...
void TpUartDataLinkLayer::isrUnlock() { _rxProcessing = false; }
...
void TpUartDataLinkLayer::forceAck(bool state) { _forceAck = true; }
...
void TpUartDataLinkLayer::enabled(bool value) {
  // After an unusual device restart, perform a reset, as the TPUart may still
 ...
 * However, if there was an invalid frame or bus disconnect, the confirmation is
 * not received and the STack is stuck in the TX_FRAME. The wait must therefore
 * be ended after a short waiting time.
 */
 ...
 * This only happens one at a time, as after each frame it is necessary to wait
 * until the frame has come in again and the L_DATA_CON comes in.
 *
 */
...
/*
 * Check whether I have not received any data for too long and set the status to
 * not connected. In normal mode, the status is requested every second. A short
 * time can therefore be selected here. In monitoring mode there are actual
 * frames, so a longer time is used here. Nevertheless, there are suspected
 * disconnects with larger data volumes, so the RxQueue is also taken into
 * account.
 */
...
void TpUartDataLinkLayer::loop() {
   ...
   * However, this only applies in the loop and not in ISR. But when using ISR
   * and DMA, this should never happen.
   */
  if (_rxOverflow) {
...
void TpUartDataLinkLayer::rxFrameReceived(TpFrame *tpFrame) {
  ...
  if (_dllcb)
    _dllcb->activity(
        ...
        (KNX_ACTIVITYCALLBACK_DIR_RECV << KNX_ACTIVITYCALLBACK_DIR));
...
DptMedium TpUartDataLinkLayer::mediumType() const { return DptMedium::KNX_TP1; }
...
void TpUartDataLinkLayer::powerControl(bool state) {
  ...
  if (state)
    _platform.writeUart(ACR0_FLAG_DC2EN | ACR0_FLAG_V20VEN | ACR0_FLAG_XCLKEN |
                        ACR0_FLAG_V20VCLIMIT);
  else
...
bool TpUartDataLinkLayer::processTxFrameBytes() {
  ...
  /*
   * Each frame must be introduced with a U_L_DATA_START_REQ and each subsequent
   * byte with a further position byte (6bit). Since the position byte consists
   * of the U_L_DATA_START_REQ + position and we start with 0 anyway, a further
   * distinction is not necessary. distinction is not necessary.
   *
   * However, the last byte (checksum) uses the U_L_DATA_END_REQ + position!
   * In addition, there is another special feature for extended frames up to 263
   * bytes long, the 6 bits are no longer sufficient. Here a U_L_DATA_OFFSET_REQ
   * + Position (3bit) must be prefixed. This means that 9 bits are available
   * for the position.
   */
 ...
 */
uint32_t TpUartDataLinkLayer::getTxFrameCounter() { return _txFrameCounter; }
/*
 * Returns the number of frames sent
 */
...
bool TpUartDataLinkLayer::isConnected() { return _connected; }

bool TpUartDataLinkLayer::isStopped() { return _stopped; }

bool TpUartDataLinkLayer::isBusy() { return _busy; }

bool TpUartDataLinkLayer::isMonitoring() { return _monitoring; }
...
#ifdef USE_TP_RX_QUEUE
/*
 * This method allows the processing of the incoming bytes to be handled
 * additionally via an interrupt (ISR). The prerequisite is that the interrupt
 * runs on the same core as the knx.loop!
 *
 * With an RP2040 where the ISR is also locked when a block is erased,
 * processing can be caught up between the erases. This significantly minimizes
 * the risk of frame losses.
 */
 ...
 * because a frame can optionally be received via an ISR and processing must
 * still take place normally in the knx.loop. In addition, this queue is
 * statically preallocated, as no malloc etc. can be made in an ISR.
 */
...
uint16_t TpUartDataLinkLayer::availableInRxQueue() {
  return ((_rxBufferFront == _rxBufferRear)
              ? (MAX_RX_QUEUE_BYTES)
              : ((((MAX_RX_QUEUE_BYTES)-_rxBufferFront) + _rxBufferRear) %
                 (MAX_RX_QUEUE_BYTES))) -
         1;
}
#endif
