...
LOG_MODULE_REGISTER(sec_if_obj, LOG_LEVEL_INF);

// FDSK is initialized to all zeros and must be set via setFDSK() from
// provisioning
uint8_t SecurityInterfaceObject::_fdsk[16] = {0};
...
SecurityInterfaceObject::SecurityInterfaceObject() {
  Property* properties[] = {
      new DataProperty(PID_OBJECT_TYPE, false, PDT_UNSIGNED_INT, 1,
                       ReadLv3 | WriteLv0, (uint16_t)OT_SECURITY),
      new CallbackProperty<SecurityInterfaceObject>(
          this, PID_LOAD_STATE_CONTROL, true, PDT_CONTROL, 1,
          ReadLv3 | WriteLv3,
          // ReadCallback of PID_LOAD_STATE_CONTROL
          [](SecurityInterfaceObject* obj, uint16_t start, uint8_t count,
      ...
      new FunctionProperty<SecurityInterfaceObject>(
          this, PID_SECURITY_MODE,
          // Command Callback of PID_SECURITY_MODE
          [](SecurityInterfaceObject* obj, uint8_t* data, uint8_t length,
             uint8_t* resultData, uint8_t& resultLength) -> void {
      ...
      new DataProperty(PID_P2P_KEY_TABLE, true, PDT_GENERIC_20, 1,
                       ReadLv3 | WriteLv0),  // written by ETS
      new DataProperty(PID_GRP_KEY_TABLE, true, PDT_GENERIC_18, 50,
                       ReadLv3 | WriteLv0),  // written by ETS
      new DataProperty(PID_SECURITY_INDIVIDUAL_ADDRESS_TABLE, true,
                       PDT_GENERIC_08, 128,
                       ReadLv3 | WriteLv0),  // written by ETS
      new FunctionProperty<SecurityInterfaceObject>(
          this, PID_SECURITY_FAILURES_LOG,
          ...
          [](SecurityInterfaceObject* obj, uint8_t* data, uint8_t length,
            ...
            if (id == 0 && info == 0) {
              ...
              resultData[1] = id;
              resultData[2] = info;
              obj->getFailureCounters(
                  &resultData[3]);  // Put 8 bytes in the buffer
              resultLength = 3 + 8;
              return;
            }
            // query latest failure by index
              ...
              uint8_t maxBufferSize =
                  resultLength;  // Remember the maximum buffer size of the
                                 // buffer that is provided to us
              uint8_t index = info;
              uint8_t numBytes = obj->getFromFailureLogByIndex(
                  index, &resultData[2], maxBufferSize);
      ...
      new DataProperty(PID_SECURITY_REPORT_CONTROL, true,
                       PDT_BINARY_INFORMATION, 1, ReadLv3 | WriteLv0,
                       _secReportCtrl),  // Not implemented
      new DataProperty(
          PID_SEQUENCE_NUMBER_SENDING, true, PDT_GENERIC_06, 1,
          ReadLv3 | WriteLv0),  // Updated by our device accordingly
      new DataProperty(PID_ZONE_KEY_TABLE, true, PDT_GENERIC_19, 1,
                       ReadLv3 | WriteLv0),  // written by ETS
      new DataProperty(PID_GO_SECURITY_FLAGS, true, PDT_GENERIC_01, 256,
                       ReadLv3 | WriteLv0),  // written by ETS
      new DataProperty(PID_ROLE_TABLE, true, PDT_GENERIC_01, 1,
                       ReadLv3 | WriteLv0),  // written by ETS
      new DataProperty(PID_ERROR_CODE, false, PDT_ENUM8, 1, ReadLv3 | WriteLv0,
                       (uint8_t)E_NO_FAULT),
      new DataProperty(PID_TOOL_SEQUENCE_NUMBER_SENDING, true, PDT_GENERIC_06,
                       1,
                       ReadLv3 | WriteLv0)  // Updated by our device accordingly
                                            // (non-standardized!)
  };
  ...
  LOG_INF("=== SecurityInterfaceObject CONSTRUCTED ===");
  LOG_HEXDUMP_INF(_fdsk, 16, "FDSK (hardcoded):");
  LOG_INF("Tool Key initialized with FDSK (will be shown after restore)");
  LOG_INF("============================================");
}
...
uint8_t SecurityInterfaceObject::getFromFailureLogByIndex(uint8_t index,
                                                          uint8_t* data,
                                                          uint8_t maxDataLen) {
...
bool SecurityInterfaceObject::isLoaded() { return _state == LS_LOADED; }

LoadState SecurityInterfaceObject::loadState() { return _state; }

void SecurityInterfaceObject::clearSecurityTables() {
  uint8_t zeroData[2] = {0, 0};
  property(PID_SECURITY_INDIVIDUAL_ADDRESS_TABLE)->write(0, 1, zeroData);
  property(PID_P2P_KEY_TABLE)->write(0, 1, zeroData);
  property(PID_GRP_KEY_TABLE)->write(0, 1, zeroData);
  LOG_INF(">>> Security tables CLEARED for fresh download");
}
...
const uint8_t* SecurityInterfaceObject::p2pKey(uint16_t addressIndex) {
  ...
  if (numElements > 0) {
    ...
    uint8_t entry[elementSize];  // 2 bytes index + keysize (16 bytes) + 2
                                 // bytes(roles) = 20 bytes
...
const uint8_t* SecurityInterfaceObject::groupKey(uint16_t addressIndex) {
  ...
  if (numElements > 0) {
    ...
    // Search for address index
    uint8_t
        entry[elementSize];  // 2 bytes index + keysize (16 bytes) = 18 bytes
...
uint16_t SecurityInterfaceObject::indAddressIndex(uint16_t indAddr) {
  ...
  uint16_t numElements =
      getNumberOfElements(PID_SECURITY_INDIVIDUAL_ADDRESS_TABLE);
...
uint64_t SecurityInterfaceObject::getSequenceNumber(bool toolAccess) {
  uint8_t seqBytes[6] = {0x00};
  Property* prop = toolAccess ? property(PID_TOOL_SEQUENCE_NUMBER_SENDING)
                              : property(PID_SEQUENCE_NUMBER_SENDING);

  if (prop->read(1, 1, seqBytes) > 0) {
    return sixBytesToUInt64(seqBytes);
  }
...
uint16_t SecurityInterfaceObject::getNumberOfElements(PropertyID propId) {
  ...
  uint8_t
      data[sizeof(uint16_t)];  // is sizeof(_currentElements) which is uint16_t
...
DataSecurity SecurityInterfaceObject::getGroupObjectSecurity(uint16_t index) {
  ...
  if (count > 0) {
    ...
    return conf   ? DataSecurity::AuthConf
           : auth ? DataSecurity::Auth
                  : DataSecurity::None;
  }
